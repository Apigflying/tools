...
tools.getDistance = (lat1, lng1, lat2, lng2) => {
  ...
  /*
    ...
    返回格式 20171223000000-20171224000000
  */
tools.getDayTimes = (datSection, nowDates = new Date()) => {
  var today = tools.redate(nowDates).split('-').join('') + '000000'
  var nearday = parseInt(tools.redate(new Date(nowDates.setDate(nowDates.getDate() + datSection))).split('-').join('')) + '000000'
  if (datSection) {
    if (datSection > 0) {
      return today + '-' + nearday
    } else {
